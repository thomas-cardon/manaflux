...
class FluxProvider extends Provider {
  ...
  async getData(champion, preferredPosition, gameMode) {
    ...
    let data = JSON.parse(await rp(`${this.base}data/v1/${champion.id}?itemsets=${Mana.getStore().get('item-sets-enable')}&summonerspells=${Mana.getStore().get('summoner-spells')}`));
