...
module.exports = {
  change: function(el) {
    log.log(2, `[UI] Changed gradient to ${this.value}`);
    $('body').css('background', `${this.value ? 'linear-gradient(to bottom, rgba(125, 185, 232, 0) -1%, rgba(50, 96, 122, 0) 65%, rgba(10, 49, 64, 0.8) 100%), ' : ''}url(./assets/img/${Mana.getStore().get('ui-background')})`);
  }
};
